...
import { applyToJob, applyToJobWithResume, getPresignedUrlForResume } from '../services/JobService';
...
const AvailableJobList = () => {
    ...
    const handleApply = async (jobId: number) => {
        if (!user?.hasUploadedGeneralResume) {
            swal.fire({
                title: 'Error',
    ...
    const handleApplyWithCV = async (jobId: number) => {
        try {
            ...
            const presignedUrl = await getPresignedUrlForResume(jobId, contentType);
            ...
            await applyToJobWithResume(jobId, contentType);
            swal.fire({
